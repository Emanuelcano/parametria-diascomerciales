	...
	*/
	...
	class BARCODE
	{
		...
		function genBarCode($barnumber,$format="gif",$file="")
		{
			...
			elseif($this->_encode=="SAETA" || $this->_encode=="INTERLEAVED SAETA")
			{ //INTERLEAVED 2 OF 5
				$this->_i25BarcodeSAETA($barnumber,$this->_scale,$file);
			}
		...
		*/
		...
		function _c93Encode($barnumber)
		{
			...
			$widesae=str_pad("",$this->_n2w,"",STR_PAD_LEFT);
		...
		function _c39Barcode($barnumber,$scale=1,$file="",$checkdigit=false)
		{
			...
			$font_arr=@imagettfbbox ( $scale*10, 0, $this->_font, $barnumber);
			$x= floor($total_x-(int)$font_arr[0]-(int)$font_arr[2]+$scale*10)/2;
			@imagettftext($im,$scale*10,0,$x, $height2, $bar_color,$this->_font , '');
		...
		*/
		...
		function _i25Barcode($barnumber,$scale=1,$file="")
		{
			...
			$x= ($total_x-strlen($bars))/2;
			@imagettftext($im,$scale*6,0,$x, $height2, $bar_color,$this->_font , '');
...
// Start Function for INTERLEAVEDSAETA
		...
		*/
		...
		function _i25EncodeSAETA($barnumber)
		{
			$encTable=array("NNWWN","WNNNW","NWNNW","WWNNN","NNWNW","WNWNN","NWWNN","NNNWW","WNNWN","NWNWN");
			$guards=array("1010","1101");
		...
		function _i25BarcodeSAETA($barnumber,$scale=1,$file="")
		{

			$bars=$this->_i25EncodeSAETA($barnumber);
			if(empty($file))
				header("Content-type: image/".$this->_format);
		...
		// End Function for INTERLEAVEDSAETA
